...
# In-process cache to reduce repeated file reads. Cache invalidated on writes.
...
def _load() -> Dict[str, Any]:
    """Load the entire database from disk using a small in-process cache.

    The cache reduces repeated JSON parsing on hot code paths. The cache is
    invalidated and refreshed whenever `_save` writes the file.
    """
    ...
    if _normalize_session_references(data):
        # Normalization modified the data; persist normalized copy.
        _save(data)
        # Reload to ensure mtime/cache consistency
        with open(DATA_FILE, "r", encoding="utf-8") as f2:
            data = json.load(f2)

    _CACHE = copy.deepcopy(data)
    _CACHE_MTIME = os.path.getmtime(DATA_FILE) if os.path.exists(DATA_FILE) else None
    return copy.deepcopy(data)


def _save(data: Dict[str, Any]) -> None:
    """Persist the entire database to disk using an atomic replace.

    Writes are done to a temporary file in the same directory and then
    atomically moved into place with `os.replace` to avoid partial-writes.
    After writing we update the in-process cache and cached mtime.
    """
...
def _init_empty() -> None:
    ...
    _save({"users": [], "sessions": [], "messages": [], "feedback": [], "tokens": []})
...
def create_token(jti: str, user_id: str) -> None:
    """Record a newly issued token (jti) for a user in the JSON store."""
    data = _load()
    data.setdefault("tokens", [])
    entry = {"jti": jti, "user_id": user_id, "created_at": now_iso(), "revoked_at": None}
    # Replace if exists
    data["tokens"] = [t for t in data["tokens"] if t.get("jti") != jti] + [entry]
    _save(data)
...
def revoke_token(jti: str) -> bool:
    data = _load()
    changed = False
    for t in data.setdefault("tokens", []):
        if t.get("jti") == jti and not t.get("revoked_at"):
